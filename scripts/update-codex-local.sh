#!/usr/bin/env bash
set -euo pipefail

CODEX_ROOT="${CODEX_ROOT:-/Users/williamxu/Desktop/Projects/codex}"
CODER_DIR="${CODEX_ROOT}/codex-rs"
LOCAL_BIN="${HOME}/.local/bin"
CURRENT_LINK_DIR="${LOCAL_BIN}"
LINK_TARGET="${LOCAL_BIN}/codex"
LINK_TARGET_TUI="${LOCAL_BIN}/codex-tui"
BACKUP_DIR="${HOME}/.local/lib/codex/releases"
STATE_DIR="${HOME}/.local/lib/codex"
STATE_FILE="${STATE_DIR}/release-tag.txt"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
NEW_TARGET_DIR="${BACKUP_DIR}/${TIMESTAMP}"
BUILD_TARGET_DIR=""
RELEASE_READY=0
WATCH_MODE=0
FORCE=0
ALLOW_DIRTY_UPDATES=0
SKIP_PULL=0
RELEASE_TAG=""
PRESERVE_LOCAL_EDITS=0
HAD_LOCAL_STASH=0
LOCAL_STASH_POPPED=0

cleanup_links() {
  rm -f "${TMP_LINK_CLI-}" "${TMP_LINK_TUI-}"
}

cleanup_build_target() {
  if [ -n "$BUILD_TARGET_DIR" ] && [ -d "$BUILD_TARGET_DIR" ]; then
    rm -rf -- "$BUILD_TARGET_DIR"
  fi
  if [ "$RELEASE_READY" -eq 0 ] && [ -d "$NEW_TARGET_DIR" ]; then
    rm -rf -- "$NEW_TARGET_DIR"
  fi
}

prune_timestamped_releases() {
  current_target="$(readlink "${HOME}/.local/lib/codex/current" 2>/dev/null || true)"
  find "$BACKUP_DIR" -mindepth 1 -maxdepth 1 -type d -name '20??????-??????' -print |
    sort -r |
    awk 'NR > 2' |
    while IFS= read -r old_release; do
      [ -n "$old_release" ] || continue
      [ "$old_release" != "$current_target" ] || continue
      rm -rf -- "$old_release"
    done
}

while [ $# -gt 0 ]; do
  case "${1:-}" in
    --watch)
      WATCH_MODE=1
      shift
      ;;
    --force)
      FORCE=1
      ALLOW_DIRTY_UPDATES=1
      shift
      ;;
    --skip-pull)
      SKIP_PULL=1
      shift
      ;;
    --allow-dirty)
      ALLOW_DIRTY_UPDATES=1
      shift
      ;;
    --preserve-local-edits)
      PRESERVE_LOCAL_EDITS=1
      shift
      ;;
    -h|--help)
      cat <<'EOF'
Usage: update-codex-local.sh [--watch] [--force]

  --watch        Check latest upstream release before building; skip if unchanged.
  --force        Force a rebuild even if the watched release tag is unchanged.
  --skip-pull    Skip fetch/pull and build current checked-out tree.
  --allow-dirty  Skip clean-tree checks and build local edits.
  --preserve-local-edits
            Temporarily stash/re-apply local edits across git pull.
  --release-tag TAG
            Use a webhook-provided release tag instead of querying GitHub.
  --help    Show this help text.
EOF
      exit 0
      ;;
    --release-tag)
      if [ -z "${2:-}" ]; then
        echo "--release-tag requires a tag"
        exit 1
      fi
      RELEASE_TAG="$2"
      WATCH_MODE=1
      shift 2
      ;;
    *)
      echo "unknown argument: $1"
      exit 1
      ;;
  esac
done

mkdir -p "$LOCAL_BIN" "$BACKUP_DIR" "$STATE_DIR"

if [ ! -d "$CODER_DIR" ]; then
  echo "Missing Codex source directory: $CODER_DIR"
  exit 1
fi

cd "$CODEX_ROOT"

extract_repo_from_remote() {
  local remote_url
  remote_url="$(git -C "$CODER_DIR" config --get remote.origin.url || true)"
  remote_url="${remote_url%.git}"
  case "$remote_url" in
    git@github.com:*) echo "${remote_url#git@github.com:}" ;;
    https://github.com/*) echo "${remote_url#https://github.com/}" ;;
    http://github.com/*) echo "${remote_url#http://github.com/}" ;;
    *) echo "" ;;
  esac
}

fetch_latest_release_tag() {
  local repo
  repo="$(extract_repo_from_remote || true)"
  if [ -n "$repo" ]; then
    local release_json
    if release_json="$(curl -fsSL "https://api.github.com/repos/${repo}/releases/latest" 2>/dev/null)"; then
      local tag
      tag="$(printf '%s\n' "$release_json" | sed -n 's/.*\"tag_name\"[[:space:]]*:[[:space:]]*\"\\([^\"]*\\)\".*/\\1/p' | head -n 1)"
      if [ -n "$tag" ]; then
        echo "$tag"
        return 0
      fi
    fi
  fi

  git -C "$CODER_DIR" ls-remote --refs --tags --sort='-v:refname' origin \
    2>/dev/null \
    | sed -n 's#.*refs/tags/##p' \
    | sed 's/\^{}//' \
    | sed '/{}$/d' \
    | sed '/^$/d' \
    | head -n 1
}

is_git_dirty() {
  [ -n "$(git -C "$CODER_DIR" status --porcelain --untracked-files=all)" ]
}

restore_local_stash() {
  if [ "$HAD_LOCAL_STASH" -eq 1 ] && [ "$LOCAL_STASH_POPPED" -eq 0 ]; then
    echo "Restoring local edits from stash..."
    if ! git -C "$CODER_DIR" stash pop --index --quiet; then
      echo "Failed to restore local edits automatically. Stash remains at stash@{0}."
      return 1
    fi
    LOCAL_STASH_POPPED=1
  fi
}

prepare_pull_with_local_edits() {
  if [ "$SKIP_PULL" -eq 1 ] || [ "$ALLOW_DIRTY_UPDATES" -eq 0 ] || [ "$PRESERVE_LOCAL_EDITS" -eq 0 ]; then
    return
  fi
  if ! is_git_dirty; then
    return
  fi

  if git -C "$CODER_DIR" stash push -u -m "codex-update-${TIMESTAMP}" >/dev/null; then
    HAD_LOCAL_STASH=1
  fi
}

reapply_local_edits() {
  if [ "$HAD_LOCAL_STASH" -eq 0 ] || [ "$LOCAL_STASH_POPPED" -eq 1 ]; then
    return
  fi
  echo "Re-applying local edits..."
  if ! git -C "$CODER_DIR" stash pop --index --quiet; then
    echo "Failed to re-apply local edits from stash after pull."
    echo "Inspect local stash with: git stash list && git stash pop"
    exit 1
  fi
  LOCAL_STASH_POPPED=1
}

trap 'cleanup_links; cleanup_build_target; restore_local_stash' EXIT

if [ "$ALLOW_DIRTY_UPDATES" -eq 0 ] && is_git_dirty; then
  echo "Working tree is dirty in $CODER_DIR; aborting."
  git -C "$CODER_DIR" status --short --untracked-files=all
  exit 1
elif [ "$ALLOW_DIRTY_UPDATES" -eq 1 ] && is_git_dirty; then
  echo "Working tree is dirty, but --allow-dirty/--force enabled; building current edits."
fi

if [ "$WATCH_MODE" -eq 1 ] && [ "$FORCE" -eq 0 ]; then
  if [ -n "$RELEASE_TAG" ]; then
    remote_release="$RELEASE_TAG"
  else
    remote_release="$(fetch_latest_release_tag || true)"
  fi
  if [ -z "$remote_release" ]; then
    echo "Could not determine remote release tag; skipping auto-update."
    exit 0
  fi
  current_release="$(cat "$STATE_FILE" 2>/dev/null || true)"
  if [ -n "$current_release" ] && [ "$current_release" = "$remote_release" ]; then
    echo "Release unchanged: $remote_release. Skipping update."
    exit 0
  fi
  echo "New release detected: ${current_release:-(none)} -> ${remote_release}. Rebuilding."
fi

if [ "$SKIP_PULL" -eq 0 ]; then
  prepare_pull_with_local_edits
  git -C "$CODEX_ROOT" fetch --all --prune --tags
  git -C "$CODER_DIR" pull --ff-only
  reapply_local_edits
else
  echo "Skipping remote fetch/pull due --skip-pull."
fi

mkdir -p "$NEW_TARGET_DIR/debug"
BUILD_TARGET_DIR="$(mktemp -d "${TMPDIR:-/tmp}/codex-local-build.XXXXXX")"

CARGO_TARGET_DIR="$BUILD_TARGET_DIR" CARGO_INCREMENTAL=0 \
  cargo build -p codex-cli -p codex-tui --manifest-path "$CODER_DIR/Cargo.toml"

BUILT_CLI="${BUILD_TARGET_DIR}/debug/codex"
BUILT_TUI="${BUILD_TARGET_DIR}/debug/codex-tui"
if [ ! -x "$BUILT_CLI" ] || [ ! -x "$BUILT_TUI" ]; then
  echo "Expected build artifacts not found:"
  echo "  $BUILT_CLI"
  echo "  $BUILT_TUI"
  exit 1
fi

cp "$BUILT_CLI" "${NEW_TARGET_DIR}/debug/codex"
cp "$BUILT_TUI" "${NEW_TARGET_DIR}/debug/codex-tui"
NEW_CLI="${NEW_TARGET_DIR}/debug/codex"
NEW_TUI="${NEW_TARGET_DIR}/debug/codex-tui"

bash "$CODEX_ROOT/scripts/test-codex-sound-path.sh"

TMP_LINK_CLI="$(mktemp "$CURRENT_LINK_DIR/.codex-cli.XXXXXX")"
TMP_LINK_TUI="$(mktemp "$CURRENT_LINK_DIR/.codex-tui.XXXXXX")"
rm -f "$TMP_LINK_CLI" "$TMP_LINK_TUI"

ln -s "$NEW_CLI" "$TMP_LINK_CLI"
ln -s "$NEW_TUI" "$TMP_LINK_TUI"
mv -f "$TMP_LINK_CLI" "$LINK_TARGET"
mv -f "$TMP_LINK_TUI" "$LINK_TARGET_TUI"

if [ ! -x "$LINK_TARGET" ]; then
  echo "Failed to update $LINK_TARGET"
  exit 1
fi

ln -sfn "$NEW_TARGET_DIR" "${HOME}/.local/lib/codex/current"
RELEASE_READY=1
prune_timestamped_releases
hash -r
echo "Updated codex links:"
echo "  $(readlink -f "$LINK_TARGET")"
echo "  $(readlink -f "$LINK_TARGET_TUI")"
echo "Version:"
"$LINK_TARGET" --version

if [ "$WATCH_MODE" -eq 1 ] && [ -n "${remote_release:-}" ]; then
  echo "$remote_release" > "$STATE_FILE"
fi
