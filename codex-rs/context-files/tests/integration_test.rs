...
use codex_context_files::{
    ContextPipeline, EntityType, PipelineBuilder, RelationshipType,
};
...
fn test_pipeline_processes_fixtures() {
    ...
    println!("  Relationships found: {}", result.stats.total_relationships);
...
fn test_extracts_people() {
    ...
    let alice_found = people.iter().any(|p| {
        p.name.contains("Alice") || p.normalized_name.contains("alice")
    });
...
fn test_extracts_technologies() {
    ...
    let has_rust = techs.iter().any(|t| {
        t.normalized_name == "rust" || t.name.to_lowercase() == "rust"
    });
...
fn test_extracts_projects() {
    ...
    if !has_dataflow {
        ...
        let any_dataflow = result.all_entities.iter().any(|e| {
            e.name.to_lowercase().contains("dataflow")
        });
...
fn test_extracts_urls_and_emails() {
    ...
    println!("Found URLs: {:?}", urls.iter().map(|u| &u.name).collect::<Vec<_>>());
    println!("Found emails: {:?}", emails.iter().map(|e| &e.name).collect::<Vec<_>>());
...
fn test_extracts_relationships() {
    ...
    let has_uses = result.all_relationships.iter().any(|r| {
        matches!(r.relationship_type, RelationshipType::Uses)
    });
    let has_created_by = result.all_relationships.iter().any(|r| {
        matches!(r.relationship_type, RelationshipType::CreatedBy)
    });
...
fn test_generates_type_based_contexts() {
    ...
    let tech_ctx = result.contexts.iter().find(|c| {
        c.context_file.concept == "technologies"
    });
...
fn test_context_summaries_are_meaningful() {
    ...
    for ctx in &result.contexts {
        ...
        println!("Context '{}' summary: {}", ctx.context_file.concept, ctx.context_file.summary);
    }
}
...
fn test_pipeline_builder() {
    ...
    for ctx in &result.contexts {
        ...
        assert!(source.is_some(), "Expected source to be set in structured data");
...
"#;
    ...
    let has_john = result.entities.iter().any(|e| {
        matches!(e.entity_type, EntityType::Person) && e.name.contains("John")
    });
    assert!(has_john, "Expected to find John Doe");

    // Should find technologies
    let techs: Vec<_> = result.entities.iter()
        .filter(|e| matches!(e.entity_type, EntityType::Technology))
        .collect();
    assert!(techs.len() >= 2, "Expected to find Rust and TypeScript");

    // Should find email
    let has_email = result.entities.iter().any(|e| {
        matches!(e.entity_type, EntityType::Email)
    });
...
fn test_chunk_structure() {
    ...
    let result = pipeline.process_document(&content, Some(&readme_path)).unwrap();
